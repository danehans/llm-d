...
Installs the GKE + agentgateway inference-scheduling stack into an existing
...
ensure_default_pool_nodes 1

if (( INITIAL_DECODE_REPLICAS > 0 )); then
  fail "INITIAL_DECODE_REPLICAS must be 0 for this script. Install the stack first, then request queued H100 capacity and use scale-components.sh to scale decode."
fi
...
if (( INITIAL_DECODE_REPLICAS > 0 )); then
  wait_for_rollout "${NAMESPACE}" "${DECODE_DEPLOYMENT}" "${INITIAL_DECODE_REPLICAS}" "${WORKLOAD_TIMEOUT_SECONDS}"
fi

log_success "Installed the GKE + agentgateway inference-scheduling stack."
